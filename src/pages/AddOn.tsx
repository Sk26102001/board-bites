...
const AddOnsPage = () => {
  ...
  const addOns = [
    {
      id: 'keepsake-boards',
      title: 'Custom Keepsake Boards',
      description: 'Laser-engraved wooden boards with guest names or event details — elegant place settings & lasting mementos.',
      price: 'AED 50 / piece',
      minQuantity: 10,
      features: ['Premium wood', 'Custom engraving', 'Gift wrapped'],
      image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=80&w=800',
    },
    {
      id: 'welcome-boxes',
      title: 'Premium Welcome Boxes',
      description: 'Luxury gift boxes for VIP guests — personalized and thoughtfully curated.',
      price: 'From AED 180 / box',
      features: ['Custom ribbon', 'Personal message', 'Premium contents'],
      image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=80&w=800',
    },
    {
      id: 'table-settings',
      title: 'Elevated Table Settings',
      description: 'Complete styling: charger plates, napkins, menus & place cards.',
      price: 'AED 35–75 / setting',
      features: ['Gold chargers', 'Foil napkins', 'Custom menus'],
      image: 'https://images.unsplash.com/photo-1551024709-8f23befc6f87?auto=format&fit=crop&q=80&w=800',
    },
    {
      id: 'dessert-table',
      title: 'Artisanal Dessert Display',
      description: 'Stunning dessert station with handcrafted sweets & elegant presentation.',
      price: 'From AED 2,500',
      features: ['French & Arabic pastries', 'Custom macarons', 'Gold stands'],
      image: 'https://images.unsplash.com/photo-1551024709-8f23befc6f87?auto=format&fit=crop&q=80&w=800',
    },
  ];
  ...
  const updateQuantity = (id: string, delta: number) => {
    setQuantities((prev) => {
      const curr = prev[id] || (addOns.find(a => a.id === id)?.minQuantity || 1);
      return { ...prev, [id]: Math.max(curr + delta, 1) };
    });
  };

  const getQuantity = (id: string) =>
    quantities[id] || (addOns.find(a => a.id === id)?.minQuantity || 1);

  return (
    <div className="min-h-screen" style={{ backgroundColor: THEME.creamBg }}>
      {/* Compact Hero */}
      <section className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <img
            ...
            alt="Elegant Add-ons"
          />
          <div className="absolute inset-0 opacity-70" style={{ backgroundColor: THEME.textMain }} />
        </div>
        <div className="relative z-10 text-center px-6">
          <h1 className="text-5xl md:text-7xl font-serif text-white mb-4">
            Add-On <span style={{ color: THEME.primarySage }}>Extras</span>
          </h1>
          <p className="text-white/90 text-lg md:text-xl mb-8 max-w-2xl mx-auto">
            Enhance your event with premium touches & keepsakes
          </p>
          <Link
            to="/cart"
            className="inline-flex items-center gap-3 px-8 py-4 rounded-full text-lg font-bold text-white hover:scale-105 transition"
            style={{ backgroundColor: THEME.primarySage }}
          >
      ...
      <section className="py-16 md:py-24 container mx-auto px-6 max-w-6xl">
        <div className="grid md:grid-cols-2 gap-10">
          {addOns.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-2xl border border-stone-100 p-6 md:p-8 hover:shadow-xl transition-shadow"
            >
              {item.image && (
                <img
                  src={item.image}
              ...
              <h3 className="text-2xl md:text-3xl font-serif mb-3" style={{ color: THEME.textMain }}>
                {item.title}
              </h3>
              <p className="text-stone-600 mb-5 text-base">{item.description}</p>

              <ul className="space-y-2 mb-6 text-sm text-stone-700">
                {item.features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2">
                    <Check className="w-4 h-4" style={{ color: THEME.primarySage }} />
                    {f}
                  </li>
                ))}
              </ul>
              ...
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mt-auto">
                <div>
                  <div className="text-2xl font-serif" style={{ color: THEME.primarySage }}>
                    {item.price}
                  </div>
                  {item.minQuantity && (
                    <div className="text-sm text-stone-500">Min: {item.minQuantity}</div>
                  )}
                </div>

                <div className="flex items-center gap-3">
                  <div className="flex border border-stone-200 rounded-full">
                    <button
                      onClick={() => updateQuantity(item.id, -1)}
                      className="px-3 py-2 hover:bg-stone-50 disabled:opacity-40"
                      disabled={getQuantity(item.id) <= (item.minQuantity || 1)}
                    >
                      <Minus className="w-4 h-4" />
                    </button>
                    <span className="px-5 py-2 font-medium">{getQuantity(item.id)}</span>
                    <button
                      onClick={() => updateQuantity(item.id, 1)}
                  ...
                  <button
                    onClick={() => alert(`Added ${getQuantity(item.id)} × ${item.title}`)}
                    className="px-6 py-2.5 rounded-lg font-medium hover:scale-105 transition"
                    style={{
                      backgroundColor: `${THEME.primarySage}15`,
                      color: THEME.primarySage,
                    }}
                  >
                    Add
        ...
        <div className="mt-16 text-center">
          <Link
            to="/cart"
            className="inline-flex items-center gap-3 px-10 py-5 rounded-full text-xl font-bold text-white hover:shadow-xl transition"
            style={{ backgroundColor: THEME.primarySage }}
          >
            Review & Checkout <ShoppingCart className="w-6 h-6" />
          </Link>
        </div>
      </section>

      <footer className="py-12 text-center border-t border-stone-100">
        <div className="text-2xl font-serif mb-2" style={{ color: THEME.primarySage }}>
          SAGE & GOLD
        </div>
        <p className="text-stone-500 text-sm">Dubai • Abu Dhabi • Sharjah</p>
      </footer>
    </div>
  );
};
