...
//     ...form,
//     total: calculateTotal(),
//     selectedItems: { ...form.selectedItems }, // deep copy
//   };
...
//           <Link
//             to="/cart"
//             className="bg-emerald-600 hover:bg-emerald-700 text-white px-10 py-4 rounded-xl font-medium text-lg shadow-md transition-all"
//           >
//             View My Carts
//           </Link>
...
//           <button
//             onClick={() => window.location.reload()}
//             className="bg-white border-2 border-gray-300 hover:bg-gray-50 text-gray-800 px-10 py-4 rounded-xl font-medium text-lg transition-all"
//           >
//             Book Another Cart
//           </button>
//         </div>
...
// useEffect(() => {
//   const editing = localStorage.getItem("editingBooking");
//   if (editing) {
//     try {
//       const data = JSON.parse(editing);
//       setForm(data);
//       // Optionally remove after loading
//       // localStorage.removeItem("editingBooking");
//     } catch (err) {
//       console.error("Failed to load editing booking", err);
//     }
//   }
// }, []);
...
const cartOptions = [
  ...
  // Drinks Cart
  {
    ...
    sections: [
      {
        id: "drink",
        title: "Non-alcholholic Champagne",
        maxSelect: 2,
        items: [
          { name: "Pink Champagne" },
          { name: "Iconic Strawberry Lemonade" },
...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    if (editing) {
      try {
        const data = JSON.parse(editing);
        setForm(data);
        setEditingId(data.id);  // Remember the ID for updating
        localStorage.removeItem("editingBooking");  // Clean up
      } catch (err) {
        console.error("Failed to load editing booking", err);
      }
    }
  ...
  const toggleItem = (sectionId: string, item: { name: string; price?: number }) => {
    if (!selectedCart) return;
  ...
  const calculateTotal = () => {
    ...
    const stationPrice = form.withStation && selectedCart?.stationPrice
      ? selectedCart.stationPrice
      : 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      // Update existing
      existingBookings = existingBookings.map((b: any) => b.id === editingId ? bookingData : b);
    ...
    console.log(editingId ? "Booking updated:" : "Booking saved:", bookingData);
            ...
            return (
              <div key={section.id} className="mb-4 overflow-hidden border border-gray-200 rounded-2xl transition-all duration-200">
                ...
                <div
                  onClick={() => setActiveSection(isOpen ? null : section.id)}
                  className={`flex items-center justify-between p-5 cursor-pointer transition-colors ${
                    isOpen ? "bg-gray-50 border-b border-gray-100" : "bg-white hover:bg-gray-50"
                  }`}
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
